...
public class GuiAok extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float particalTick) {
        ...
        if(this.ieep.getLandPos().equals(new BlockPos(0, 0, 0)) && this.ieep.getLordLevel() == 0) {
            ...
            this.drawString(fontRendererObj, "BlockPos : x=" + ieep.getLandPos().getX() + ", y=" + ieep.getLandPos().getY() + ", z=" + ieep.getLandPos().getZ(), 10, 10, 0xffffff);
            this.drawString(fontRendererObj, "Lord Level : Lv. " + ieep.getLordLevel(), 10, 20, 0xffffff);
            this.drawString(fontRendererObj, "Lord Name : " + ieep.getLordName(), 10, 30, 0xffffff);
            this.drawString(fontRendererObj, "Member Count : " + ieep.getMembers().size(), 10, 40, 0xffffff);
        }
    }
}
